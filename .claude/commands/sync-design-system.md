...
1. Run `bun run design:lint` — must return zero errors. If there are contrast warnings, surface them and ask before proceeding.
2. Run `bun run design:export` — regenerates `tailwind.theme.json` from DESIGN.md.
...
4. Run `bun run build` to verify the change compiles.
5. Run `bun run test:e2e` to verify visual regression doesn't fire (Playwright snapshots).
