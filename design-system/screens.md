...
- `/dean-decision/:token` — Session-less Dean decision page reached via the `decision_token` in the "New weekend request" email, for both registered-requester and guest requests routed to a Dean. Shows requester (with an "External" badge for guests), key or requested room, date, and the uploaded letter/stamp if any, with Approve/Decline buttons; a guest request additionally shows a key picker that must be filled before Approve is enabled. GET never mutates state (mail-scanner safety); the decision happens only on an explicit button click, which `POST`s.
