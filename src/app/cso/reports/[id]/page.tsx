...
// See docs/AI.md's ReportProvenance table — what each fallback_reason means
// and whether regenerating is likely to help.
const FALLBACK_REASON_COPY: Record<string, string> = {
  no_api_key:
    'Gemini API key not configured — regenerating will not help until this is fixed in deployment.',
  sdk_error:
    'Gemini call failed (network, quota, or model error) — usually transient; regenerating may produce an AI-written report.',
  unparseable_output:
    "Gemini responded but the output didn't match the expected format — regenerating may or may not resolve it.",
};
...
export default async function ReportDetailPage({
  ...
  const fallbackReason = report.metadata?.fallback_reason as
    | string
    | null
    | undefined;
  const fallbackReasonCopy = fallbackReason
    ? FALLBACK_REASON_COPY[fallbackReason]
    : undefined;
            ...
            <div className="mt-6 flex flex-col gap-2 border-t border-border pt-4 text-xs text-muted-foreground">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {source === 'template' ? (
                  <Badge
                    variant="secondary"
                    aria-label="Template-generated report (AI unavailable)"
                  >
                    <FileTextIcon aria-hidden="true" />
                    Template fallback
                  </Badge>
                ) : (
                  <Badge variant="outline" aria-label="AI-generated report">
                    <SparklesIcon aria-hidden="true" />
                    AI-generated
                  </Badge>
                )}
              </div>
              {source === 'template' && fallbackReasonCopy && (
                <p>{fallbackReasonCopy}</p>
              )}
            </div>
          </section>
