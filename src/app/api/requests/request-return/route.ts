...
const mapRpcError = (msg: string): { status: number; message: string } => {
  ...
  if (msg.includes('NOT_ISSUED'))
    return { status: 409, message: 'This key is not currently issued to you' };
...
export const POST = async (request: NextRequest) => {
  ...
  if (rpcError) {
    ...
    if (mapped.status === 500) {
      ...
      logger.error('request_return RPC failed', {
  ...
  if (!result) {
    const ref = crypto.randomUUID();
    logger.error('request_return RPC returned empty result', { ref });
    return NextResponse.json(err(`Internal error. Ref: ${ref}`, 500), {
      status: 500,
    });
  }
