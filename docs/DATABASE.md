...
- Index `requests_one_live_issue_per_key` (`20260904150000_issue_key_single_holder_guard.sql`): partial unique index on `(key_id) where status = 'KEY_ISSUED'` — at most one live holder per key, since a key record is one physical bunch. This is the structural half of the guard in `issue_key` (below); the RPC's own check exists to produce a clean 409 message, but the index is what survives a concurrent collection, where two `issue_key` calls on different requests for one key would each pass an `exists` test (each call's `for update` locks only its own request row). `key_id` is nullable, but `requests_key_required_after_pending` already forbids a null one in `KEY_ISSUED`, and nulls never conflict in a unique index
...
- `issue_key(request_id, verifier_id)` — flips request status, sets issued_at, audit entry. Refuses (`CONFLICT`, `P0006`, → 409) when another request for the same key is already `KEY_ISSUED`, so one physical key can only be out with one person at a time. Before `20260904150000_issue_key_single_holder_guard.sql` it validated only the request row: `create_request`'s duplicate check is scoped to `r.requester_id`, so a second requester could get a valid code for a key already out and be handed a key that wasn't on the hook. The guard locks the `keys` row (lock order requests → keys, matching `return_key`) and is backed by the `requests_one_live_issue_per_key` index below, which is what actually closes the concurrent-collection race. Note the guard is strict rather than a capacity check against `keys.key_count` — that column is the number of keys on the one bunch this record represents, not a count of interchangeable copies.
