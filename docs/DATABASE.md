...
- `status` enum: 'PENDING_HOD' (weekend only) | 'APPROVED' (weekend only — Dean approved, awaiting on-the-day code) | 'CODE_ISSUED' | 'KEY_ISSUED' | 'KEY_RETURNED' | 'EXPIRED' | 'CANCELLED' | 'DECLINED'. The live `request_status` type also carries three legacy values — `PENDING`, `SUCCESS`, `ERROR` — left over from an early bootstrap migration (Postgres can't cheaply drop enum values). No current code path reads or writes them; treat as dead.
...
> **Deploy gap (confirmed 2026-08-29 via live `list_migrations`/`list_tables`)**: the two
> migrations backing this section — `20260820120000_audit_export_state.sql` and
> `20260820120100_audit_log_export_cron.sql` — exist in `supabase/migrations/` but have
> **never been applied to the production database**. `audit_export_state` does not exist live,
> and no `audit-log-export` pg_cron job is registered (`cron.job` lists only the other six).
> `POST /api/cron/audit-export` (code exists, documented in `docs/API.md`) would fail if
> invoked today. Everything below describes the intended/designed behaviour, not current
> production state — run `bun run db:migrate` against production (or apply via the Supabase
> dashboard/CLI) to close this gap, then remove this note.
