...
- `status` enum: 'PENDING_HOD' (weekend only) | 'APPROVED' (weekend only — Dean approved, awaiting on-the-day code) | 'CODE_ISSUED' | 'KEY_ISSUED' | 'KEY_RETURNED' | 'EXPIRED' | 'CANCELLED' | 'DECLINED'. A prior version of this doc claimed the live `request_status` type also carries three legacy values — `PENDING`, `SUCCESS`, `ERROR` — but a full grep of every migration file in `supabase/migrations/` finds no `CREATE TYPE`/`ALTER TYPE` statement introducing any of the three, including the migration this doc named as their source (`20260525000000_local_bootstrap_table_grants.sql`, which contains only `GRANT` statements). Either this was true only of the live production enum and never captured in a migration (an undocumented deploy gap, same class as the `audit_export_state` note below) or the original claim was simply wrong — unverified from source. Treat as unconfirmed until checked against the live DB (`select enum_range(null::request_status)`), not as fact.
