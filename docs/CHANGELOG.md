...
- **Verified working**: `requests` (13 subscribers) and `keys` (2) are both in the `supabase_realtime` publication with `REPLICA IDENTITY FULL`; `audit_log` (2) is published with default replica identity, which is correct — both its subscribers use `onInsert` only, and INSERT always carries the full new row, so FULL would add WAL cost for nothing.
...
- **Migration needs applying** — it is a file only until `npm run db:migrate` runs against the live project.
