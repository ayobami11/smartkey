...
// "+" address tags (`smartkey.e2e.tests+cso@gmail.com`, `+dean@`,
// `+verifier@`) — Gmail delivers all three to the same inbox, so one IMAP
// credential pair covers every role. `toAddress` filters which role's code
// to read back when multiple logins are in flight (Playwright runs
// `fullyParallel`).
...
export const fetchOtpCode = async ({
  ...
  throw new Error(
    `No OTP email arrived for ${toAddress} within ${POLL_TIMEOUT_MS / 1000}s. ` +
      'Check GMAIL_USER/GMAIL_APP_PASSWORD on the app under test, and that ' +
      `${toAddress} is a real "+"-tagged alias of E2E_OTP_IMAP_USER.`
  );
};
